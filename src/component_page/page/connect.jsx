...
const WALLETS = [
  { name: 'MetaMask', icon: metamask, description: 'Connect to your MetaMask Wallet', type: 'metamask' },
  { name: 'WalletConnect', icon: walletconnect, description: 'Scan with WalletConnect to connect', type: 'walletconnect' },
  { name: 'Coinbase Wallet', icon: coinbase, description: 'Connect to your Coinbase Wallet', type: 'coinbase' },
  { name: 'Trust Wallet', icon: trustwallet, description: 'Connect to your Trust Wallet', type: 'trust' }
];
      ...
      <Box sx={{ position: 'fixed', top: 0, left: 0, width: '100%', height: '100%', bgcolor: 'rgba(0,0,0,0.5)', display: 'flex', justifyContent: 'center', alignItems: 'center', zIndex: 1300 }}>
        <Box sx={{ width: '90%', maxWidth: 500, bgcolor: 'background.paper', borderRadius: 2, boxShadow: 24, p: 3, position: 'relative' }}>
          <Box display="flex" justifyContent="space-between" alignItems="center" mb={2}>
            <Typography variant="h6" sx={{color:'black'}}>Kết nối ví</Typography>
            ...
            {WALLETS.map((wallet) => (
              <ListItem key={wallet.type} disablePadding>
                <ListItemButton onClick={() => handleConnectWallet(wallet.type)} disabled={connecting} sx={{ border: '1px solid', borderColor: 'divider', borderRadius: 1, mb: 1, '&:hover': { bgcolor: 'action.hover' } }}>
                  ...
                  <ListItemText primary={wallet.name} sx={{color:'black', fontWeight:'bold'}} secondary={wallet.description} />
                </ListItemButton>
              </ListItem>
            ))}
          </List>
